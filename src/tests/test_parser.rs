...
fn parser_unit_node_binary_expr() {
    ...
    match &*ast[0] {
        Node::BinaryExpr(top_expr) => {
            assert_eq!(top_expr.op, "+", "BinOp Precedence issue: expected top expression operator to be a plus.");

            match &*top_expr.left {
                Node::BinaryExpr(left_expr) => {
                    assert_eq!(left_expr.op, "/", "BinOp Precedence issue: expected left expr operator to be a division.");

                    match &*left_expr.left {
                        Node::BinaryExpr(mul_expr) => {
                            assert_eq!(mul_expr.op, "*", "BinOp Precedence issue: expected lowest-left expr operator to be multiplicative.");
                    ...
                    match &*left_expr.right {
                        Node::NumericLiteral(lit) => assert_eq!(lit.literal_value.parse::<i32>().unwrap(), 1),
            ...
            match &*top_expr.right {
                Node::NumericLiteral(lit) => assert_eq!(lit.literal_value.parse::<i32>().unwrap(), 2),
...
fn parser_unit_node_while_stmt_comparator() {
    ...
    match &**ast.first().unwrap() {
        Node::WhileStmt(stmt) => {
            match stmt.condition.as_ref() {
                Node::Comparator(comp) => {
                    match &*comp.lhs {
                        Node::Identifier(ident) => {
                            assert_eq!(ident.identifier_name, "CONDITION_LEFT", "LHS of comparator has incorrect precedence")
                        }
                        _ => panic!("Expected a CONDITION_LEFT Identifier node for LHS of comparator")
                    }
                    match &*comp.rhs {
                        Node::Identifier(ident) => {
                            assert_eq!(ident.identifier_name, "CONDITION_RIGHT", "RHS of comparator has incorrect precedence")
                        }
                        _ => panic!("Expected a CONDITION_RIGHT Identifier node for RHS of comparator")
                    }
                    assert_eq!(comp.op, "==", "Operator for comparator expected to be DoubleEq")
                }
...
fn parser_unit_node_while_stmt_truthy() {
    ...
    match &**ast.first().unwrap() {
        Node::WhileStmt(w) => {
            match &*w.condition {
                Node::Identifier(ident) => {
                    assert_eq!(ident.identifier_name, "CONDITION_LEFT", "Expected identifier name for while condition expr");
                }
...
fn parser_unit_node_string_literal() {
    ...
    match &**ast.first().unwrap() {
        Node::StringLiteral(s) => {
            assert_eq!(s.literal_value, "Hello World", "String literal inner parsing failure: incorrect characterset")
        }
...
fn parser_unit_node_object_lieral() {
    ...
    match &**ast.first().unwrap() {
        Node::ObjectLiteral(obj) => {
            ...
            assert!(obj.props.get("a").is_some());
            assert!(obj.props.get("b").is_some());

            match &**obj.props.get("a").unwrap() {
                Node::ObjectLiteral(obj2) => {
                    assert!(obj2.props.get("sub_object").is_some());
...
fn parser_unit_node_list_literal() {
    ...
    match &**ast.first().unwrap() {
        Node::ListLiteral(listlit) => {
            ...
            match &**listlit.props.get(0).unwrap() {
            ...
            match &**listlit.props.get(2).unwrap() {
                Node::ListLiteral(listlit2) => {
                    ...
                    match &**listlit2.props.get(0).unwrap() {
